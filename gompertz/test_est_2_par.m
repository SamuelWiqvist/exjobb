...
plot(sigma_a1)
%% Est sigma tau with AIF 
[theta_obs_A, log_lik_vec, nbr_not_a, eig_vec, norm_vec] = IF1_gompertz_v2_Hessian_est_var(log(theta0(2:3)),exp(Y_hat), M_A, N,J_seq_A,tau_seq_A,sigma_seq_A,Sigma,k_true, true, a_m,c_m,burn_in);

exp(theta_obs_A(end,:))
figure
subplot(211)
plot(exp(theta_obs_A(10:end-1,1)))
title('AIF')
subplot(212)
plot(exp(theta_obs_A(10:end-1,2)))
%% Est sigma tau with IF1 
[theta_obs_changeJ, loglikchangeJ] = IF1_gompertz_v2_est_var(log(theta0(2:3)),exp(Y_hat), M, N,J_seq,tau_seq,a_seq,sigma_seq,Sigma,k_true, true);
...
%% Est sigma tau with IF2 

theta_obs_IF2 = IF2_gompertz_est_var(log(theta0_IF2_est_var),exp(Y_hat), M, sigma_a1,J,N,k_true);

sigma_IF2 = exp(mean(theta_obs_IF2(:,1:J),2));
tau_IF2 = exp(mean(theta_obs_IF2(:,J+1:2*J),2));

[sigma_IF2(end) tau_IF2(end)]
