function [theta_obs, log_lik_vec, nbr_not_a, eig_vec, norm_vec] = IF1_gompertz_v2_Hessian_est_var(theta0_log,Y_hat, M, N,J_seq,tau_seq,sigma_seq,Sigma,par_K, print_m, a_m,c_m,burn_in)
    % Description: 
    ...
    theta_bar_vec_add_log = zeros(N,2);
    theta_bar_vec_sub_log = zeros(N,2);

    % vectors to store the variance of theta values in 
    theta_var_vec_log = zeros(N,2);
    theta_var_vec_add_log = zeros(N,2);
    theta_var_vec_sub_log = zeros(N,2);

    % draw delta_k from bernoulli +-1 dist  
    delata_k_sigma = binornd(1,0.5,1,M);
    delata_k_theta = binornd(1,0.5,1,M);
    delata_k = [delata_k_sigma', delata_k_theta'];   
    delata_k(delata_k == 0) = -1;

    % calc c_k * delta_k 
    c_delat_k = [c_m'.*delata_k(:,1) c_m'.*delata_k(:,2)];
...
%             theta_bar_vec_sub_log(n,:) = mean(theta_F_log_sub);
        ...
        % calc delta G_k 
        theta_var_vec_add_log_sigma = theta_var_vec_add_log(:,1).^(-1);
        theta_var_vec_add_log_tau = theta_var_vec_add_log(:,2).^(-1);

        theta_var_vec_sub_log_sigma = theta_var_vec_sub_log(:,1).^(-1);
        theta_var_vec_sub_log_tau = theta_var_vec_sub_log(:,2).^(-1);

        delta_G_k = [sum( theta_var_vec_add_log_sigma.*(theta_bar_vec_log(1:N,1) - [theta_bar_0_add_log(1); theta_bar_vec_log(1:end-1,1)])) - ...
                    sum( theta_var_vec_sub_log_sigma.*(theta_bar_vec_log(1:N,1) - [theta_bar_0_sub_log(1); theta_bar_vec_log(1:end-1,1)])), ...
                    sum( theta_var_vec_add_log_tau.*(theta_bar_vec_log(1:N,2) - [theta_bar_0_add_log(2); theta_bar_vec_log(1:end-1,2)])) - ...
                    sum( theta_var_vec_sub_log_tau.*(theta_bar_vec_log(1:N,2) - [theta_bar_0_sub_log(2); theta_bar_vec_log(1:end-1,2)]))]';
        ...
        d_k_vec = 10000*(1:0.5:M);
        
        d_k = d_k_vec(m);

        %H_bar_bar_k = real(sqrtm(H_bar_k*H_bar_k)) + eye(1);
        H_bar_bar_k = H_bar_k + d_k*eye(2);

        eig_vec(:,m) = eig(H_bar_bar_k);
        norm_vec(m) = norm(H_bar_bar_k);
